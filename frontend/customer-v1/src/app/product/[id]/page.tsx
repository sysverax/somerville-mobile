...
import { ChevronRight, Share2, Truck, Shield, RotateCcw, Package, Wrench, Clock, Calendar, Sparkles, RefreshCw, CheckCircle2, DollarSign } from "lucide-react";
import {
...
const ProductDetailPage = ({ params }: Props) => {
  ...
  const groupedServices = useMemo(() => {
    // Non-variants are the potential parents or standalone services
    ...
    nonVariants.forEach(service => {
      const serviceVariants = variantMap.get(service.serviceId || (service as any).id) || [];
      if (serviceVariants.length > 0) {
        groups.push({ type: "parent", parent: service, variants: serviceVariants });
      } else if (!service.isParent) {
        // Only push as a single service if it's not a parent marker without variants (logic check)
                    ...
                    {groupedServices.map((group) => {
                      if (group.type === "single") {
                        ...
                        return (
                          <div
                            key={service.id}
                            className="inline-block w-full mb-6 break-inside-avoid p-6 rounded-xl bg-gradient-card border border-border hover:border-primary/50 transition-colors shadow-sm"
                          >
                            ...
                            <Button
                              className="w-full bg-gradient-primary hover:opacity-90 text-primary-foreground gap-2 transition-all shadow-md active:scale-95"
                            ...
                            {variants.map(variant => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full text-left rounded-lg border p-3 transition-all outline-none ${isSelected ? "border-primary bg-primary/10 shadow-sm" : "border-border hover:border-primary/40"}`}
                                >
